...
class EditTransferPage extends StatefulWidget {
  ...
  const EditTransferPage({super.key, required this.transferId});
...
class _EditTransferPageState extends State<EditTransferPage> {
  ...
  void initState() {
    super.initState();
    context.read<TransferBloc>().add(TransferEditInit(widget.transferId));
  }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<TransferBloc, TransferState>(
      listener: (context, state) {
        if (state is TransferReady && !_initialized) {
          _initialized = true;
          _amountController.text = state.amountText.isNotEmpty
              ? ThousandsInputFormatter.formatForDisplay(
                  double.tryParse(state.amountText) ?? 0)
              : '';
          _feeController.text = state.feeText.isNotEmpty
              ? ThousandsInputFormatter.formatForDisplay(
                  double.tryParse(state.feeText) ?? 0)
              : '';
...
// ─── Sub-widgets (reused same pattern as AddTransferPage) ───────────────────
